...
The "description", isn't the real DOM, it's actually a double (aka
mock/fake/stub).
...
These are implementation details that the user doesn't care about. They don't
care that a component named `PublishModal` is rendered. They care that they can
see journals and can click on publish buttons.

These points are made in [this blog post](https://kentcdodds.com/blog/why-i-never-use-shallow-rendering).
</div>
...
RTL is a utility library that encourages good testing practices.

The [guiding principles](https://testing-library.com/docs/guiding-principles)
state that using DOM node is better than component instances. So it doesn't use
shallow rendering.
...
Because of this principle, the API forces you to operate the application via
relatively limited "queries". These queries (e.g. `getByText()`) can only be
used like a user would think about using the application.
...
it('renders', () => {
  ...
  // We can fix by adding <label>Some label text</label> to our component
...
<small>You can see the source for this presentation [here]()</small>
